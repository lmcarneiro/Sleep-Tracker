...
def plot(df_list, smooth=False, tp=None):
    ...
    for df in df_list:
        ...
        if smooth is True:
            ...
            ax5.scatter(datetimes[df.Awake == 0], df.PR[df.Awake == 0], color='green')
            ax5.scatter(datetimes[df.Awake == 1], df.PR[df.Awake == 1], color='red')
            #ax5.scatter(datetimes, df.PR_diff)
            ax4.plot(datetimes, df.PR_delta, color='gold')
    
    print("\nPlots finished!")

def time_asleep(dates, load_all=False, tp=None, plotit=False, threshold=0,
                average=False, who='Me'):
    """Calculates time spent asleep based on HR and movement. Want to calculate
    the time spent in each stage too."""
    ...
    """First stage is light
       ...
       HR drops during sleep to adapt to lower metabolic needs. Consequently,
       HR drops even more from light sleep to deep sleep.
    ...
    for df in dates_df:
        ...
        #thresh_t = d8.datetime(thresh_t)
        
        avg_t = time.gmtime(sum(df.Awake*4))
        avg_t_str = time.strftime('%H:%M:%S', avg_t)
        
        tot_time_ts = tot_time.total_seconds()
        thresh_sleep = time.strftime('%H:%M:%S', time.gmtime(tot_time_ts - sum(df.PR_awake*4)))
        avg_sleep = time.strftime('%H:%M:%S', time.gmtime(tot_time_ts - sum(df.Awake*4)))
        
        print()
        print("\nTime awake threshold: %s" %thresh_t_str)
